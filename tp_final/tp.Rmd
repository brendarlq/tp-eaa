...
# Eliminar columnas irrelevantes como id-sum
delitos_limpio <- delitos %>%
  select(-`id-sum`)  # Asegúrate de que esta variable no está en el modelo


# Convertir variables categóricas a factores y verificar
delitos_limpio <- delitos_limpio %>%
  mutate(across(where(is.character), as.factor))

str(delitos_limpio)  # Verificar la estructura

# Entrenar el modelo Random Forest
modelo_rf <- randomForest(tipo ~ ., data = delitos_limpio, importance = TRUE)
