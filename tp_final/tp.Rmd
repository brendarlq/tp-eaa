...
# 2. Agrupación por barrio y fecha
# Agrupar por barrio y fecha y contar delitos
delitos_agrupados <- delitos %>%
  group_by(barrio, fecha) %>%
  summarise(
    frecuencia = n(),  # Contar filas
    .groups = "drop"
  )

# Revisar los datos agregados
head(delitos_agrupados)

# 3. Creación de la matriz funcional
# Crear matriz funcional con fechas como columnas
delitos_matrix <- delitos_agrupados %>%
...
# Normalizar las funciones (opcional)
delitos_matrix_scaled <- scale(delitos_matrix)

# Revisar la variabilidad de las filas
variabilidad <- apply(delitos_matrix_scaled, 1, var)
print(variabilidad)

# Filtrar barrios con baja variabilidad
delitos_matrix_scaled <- delitos_matrix_scaled[variabilidad > 0.01, ]

# Validar que hay suficientes observaciones para clustering
if (nrow(delitos_matrix_scaled) < 3) {
  stop("No hay suficientes barrios con datos válidos para realizar clustering.")
}

# Convertir a objeto funcional
fdata_obj <- fdata(delitos_matrix_scaled)

# Clustering funcional con kmeans.fd
set.seed(123)  # Fijar semilla para reproducibilidad
clustering_result <- kmeans.fd(fdata_obj, ncl = 5, par.ini = list(method = "kmeans++"))

# Asignar clusters a los barrios
clusters <- data.frame(barrio = rownames(delitos_matrix_scaled), cluster = clustering_result$cluster)

# 5. Visualización de curvas funcionales
# Crear un dataframe para visualización
df_curves <- as.data.frame(delitos_matrix_scaled) %>%
  rownames_to_column("barrio") %>%
  pivot_longer(-barrio, names_to = "dia", values_to = "frecuencia")

# Graficar las curvas funcionales por cluster
ggplot(df_curves, aes(x = dia, y = frecuencia, group = barrio, color = as.factor(clusters$cluster))) +
  geom_line(alpha = 0.7) +
  theme_minimal() +
  labs(title = "Clustering Funcional por Barrio", x = "Día", y = "Frecuencia", color = "Cluster")
